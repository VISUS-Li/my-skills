...
2. Use GPT Image models or other image generators only for visual plates that do not need exact readable text. **Prompt 默认中文**（见 `visual-asset-generation.md`）；批量生成，宁多勿少。
...
- If the frame is mainly mood, metaphor, background, or illustrative B-roll, route to image/video generation — **用中文 prompt，一次规划多素材填满各层**。
